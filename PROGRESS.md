...
## Student Portal (feat/student-portal)
- [x] **Admin-managed accounts + semesters + sessions** — Admin console at `/dashboard/admin` (create student/teacher IDs → Supabase invite link, users list, session panel). Invited users complete setup at `/auth/set-password` (must-change-password gate in middleware). Students have immutable `student_id` + `current_semester` (1.1…4.2); "Start next session" advances all active students one step (4.2 graduates). Course apply restricted to the student's current semester (derived from course code, e.g. CSE 3201/321 → 3.2). SQL: `supabase/admin_student_system.sql`. · verified: `npm run build` passes. · **Needs one-time SQL run.** · uncommitted
- [x] **Student portal at `/dashboard/student`** — Overview & Profile (profile card + metric counts + quick links), Exam Marks & Grades (answer_scripts+grades by student name, examiner feedback, rubric, **Request Regrade** → `grade_requests`), Course Applications (catalog from `courses` + per-student status, **Apply** → `course_applications`). Role-gated (student/admin), redirect to `/dashboard/student` on login, student identity badge in Navbar. API: `/api/student/overview|grades|courses`. Seed: `npm run seed:student` (student@facultyos.edu / Student@12345, name matches seeded scripts). · verified: `npm run build` passes; browser-tested overview + grades + regrade appeal (wrote `req-...` to grade_requests). · **Needs one-time SQL: `supabase/course_applications.sql`** for the apply flow. · uncommitted
